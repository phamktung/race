...
import {getCategorys, getPostsByTax} from "../../src/utils/blog";
...
const PostsCategory = ( { headerFooter, postData } ) => {
      ...
      <div className="row">
...

        {
          postData.map( ( post, index ) => {
            //console.log(postData?.attachment_image?.img_src[0])
            return (
              <div
                key={ `${ post?.id ?? '' }-${ index }` ?? '' }
                className="col-lg-4"
...
export async function getStaticProps( { params } ) {
  ...
  const postData = await getPostsByTax( params?.slug ?? '' );
  //console.log('postData',postData);
  const defaultProps = {
    props: {
      headerFooter: dataLayout?.data ?? {},
      postData: postData.data.data.posts ?? {}
    },
    /**
